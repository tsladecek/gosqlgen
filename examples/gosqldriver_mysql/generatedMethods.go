...
func (t *User) getByPrimaryKeys(ctx context.Context, db dbExecutor, _id int) error {
	err := db.QueryRowContext(ctx, "SELECT _id, id, name, payload, age, drives_car FROM users WHERE _id = ?", _id).Scan(&t.RawId, &t.Id, &t.Name, &t.payload, &t.Age, &t.DrivesCar)
...
func (t *User) getByBusinessKeys(ctx context.Context, db dbExecutor, id string) error {
	err := db.QueryRowContext(ctx, "SELECT _id, id, name, payload, age, drives_car FROM users WHERE id = ?", id).Scan(&t.RawId, &t.Id, &t.Name, &t.payload, &t.Age, &t.DrivesCar)
...
func (t *User) insert(ctx context.Context, db dbExecutor) error {
	res, err := db.ExecContext(ctx, "INSERT INTO users (id, name, payload, age, drives_car) VALUES (?, ?, ?, ?, ?)", t.Id, t.Name, t.payload, t.Age, t.DrivesCar)
	if err != nil {
		return err
	}
...
func (t *User) updateByPrimaryKeys(ctx context.Context, db dbExecutor) error {
	_, err := db.ExecContext(ctx, "UPDATE users SET name = ?, payload = ?, age = ?, drives_car = ? WHERE _id=?", t.Name, t.payload, t.Age, t.DrivesCar, t.RawId)
	return err
}

func (t *User) updateByBusinessKeys(ctx context.Context, db dbExecutor) error {
	_, err := db.ExecContext(ctx, "UPDATE users SET name = ?, payload = ?, age = ?, drives_car = ? WHERE id=?", t.Name, t.payload, t.Age, t.DrivesCar, t.Id)
	return err
}
