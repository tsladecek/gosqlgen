...
func (d driver) Create(w io.Writer, table *gosqlgen.Table, methodName string) error {
	...
	if aiCol != nil {
		if !slices.Contains(gosqlgen.IntegerTypes, aiCol.Type.String()) {
			return fmt.Errorf("autoincrement column %s must be one of following types: %v", aiCol.Name, gosqlgen.IntegerTypes)
		}
